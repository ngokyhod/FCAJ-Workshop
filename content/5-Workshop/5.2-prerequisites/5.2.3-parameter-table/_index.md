...
Each member fills in values after creation — **share with the whole team** (Google Sheet / shared file).
...
```
Trí → Kiệt → Toàn → Hoa → Trinh → 5.8 (shared testing)
```

{{% notice info %}}
You may **pause Toàn** (backend not deployed yet) and do **Trinh** first to write the report — leave testing for section 5.8.
{{% /notice %}}
